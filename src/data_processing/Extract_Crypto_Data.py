...
def fetch_historical_crypto_data(crypto_pair_sym: str, exchange_name: str,
                                 timeframe: str = '15m', limit: int = None, 
                                 since: int = None, until: int = None, save_path: str = None, save: bool = False) -> pd.DataFrame:
    '''
        ...
        Parameters:
            ...
            save (bool): Boolean to decide whether to save the fetched data or not.
    ...
    if since and until:
        ...
        if limit:
            ohlcv = exchange.fetch_ohlcv(symbol=crypto_pair_sym, timeframe= timeframe, limit = limit)
        else:
            ohlcv = exchange.fetch_ohlcv(symbol=crypto_pair_sym, timeframe= timeframe)
    
        
    ohlcv_df = pd.DataFrame(all_ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    
    # Filter the data to ensure it's within the correct timespan and convert to readable timestamp
